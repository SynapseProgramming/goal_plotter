...
public:
    ...
    Node("sub_goal_node")
  {
    ...
    auto goal_callback= [this](const geometry_msgs::msg::PoseStamped::SharedPtr msg){
       ...
       RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"[get_goal] Received goal values: x: %f y: %f z: %f w: %f",msg->pose.position.x,msg->pose.position.y,msg->pose.orientation.z,msg->pose.orientation.w);
    ...
    auto send_goal= [this](const std::shared_ptr<goal_plotter::srv::Sgoal::Request> request, const std::shared_ptr<goal_plotter::srv::Sgoal::Response> response){
          data=request->data;
    ...
    goal_service = this->create_service<goal_plotter::srv::Sgoal>("get_curr_goal", send_goal);
  }
